...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define LOG_YMODEM_ENABLE           (LOG_ENABLE && 1)
#if LOG_YMODEM_ENABLE == 1
#define LOG_YMODEM(fmt, ...)        LOG(fmt, ##__VA_ARGS__)
#else
      ...
      switch (ReceivePacket(aPacketData, &packet_length, DOWNLOAD_TIMEOUT))
      {
        case HAL_OK:
          ...
          switch (packet_length)
          {
            ...
            default:
              /* Normal packet */
              LOG_YMODEM("pkgLen=[%d] pkg[2]=[0x%02x], pkgRecv=[0x%02x]\r\n", packet_length, aPacketData[PACKET_NUMBER_INDEX], packets_received);
              ...
              else
              {
                if (packets_received == 0)
                {
                  ...
                  if (aPacketData[PACKET_DATA_INDEX] != 0)
                  {
                    ...
                    {
                      /* End session */
                      LOG_YMODEM("file size out of range, send CA & CA\r\n");
                      tmp = CA;
                    ...
                    LOG_YMODEM("Flash Erase All...\r\n");
                    FLASH_If_Erase(APPLICATION_ADDRESS, 0xFFFFFFFF);
                    *p_size = filesize;

                    LOG_YMODEM("send ACK & CRC16\r\n");
                    Serial_PutByte(ACK);
                ...
                else /* Data packet */
                {
                  ...
                  {
                    LOG_YMODEM("flashWrite at [0x%08x] succ.", flashdestination);
                    LOG_YMODEM("FirWord[0x%08x], lastWord[0x%08x]\r\n", *(uint32_t*)ramsource, *(uint32_t*)&aPacketData[packet_length]);
                  ...
                  {
                    LOG_YMODEM("flashWrite at [0x%08x] failed.\r\n", flashdestination);
                    /* End session */
                    Serial_PutByte(CA);
                    Serial_PutByte(CA);
                    result = COM_DATA;
